...
@Service
public class EmailRegistrationBPartnerService {
    ...
    @Transactional
    public String registerBPartner(RegistrationRequest request) {
        boolean isValidEmail = emailValidator.test(request.getEmail());
        if(!isValidEmail){
            throw new IllegalStateException("email not valid");
        }

        String token = userService.savedBPartner(
                new User(
        ...
        String link = "http://localhost:8082/bunisess-partner/register/confirm?token=" + token;
    ...
    public String confirmToken(String token){
        ...
        userService.enabledBPartner(
                confirmationToken.getUser().setEnabled(true));


        return "Vous êtes désormais administrateur de Camtougo!";
    }

    private String buildEmail(String name, String link, String policy) {
        return "<h1> Camtougo confirmation </h1> " +
                "<p>Bonjour  " + name + ", </p>" +
                "<p>Lisez les confidentialités de Camtougo en cliquant sur le lien suivant. Ensuite vérifiez votre compte administrateur si vous être d'accord en cliquant sur le dernier lien.</p>" +
                "<div><a href=" + policy + ">" + "données de confidentialités</a></div>" +
                "</br>" +
                "<div><a href=" + link + ">" + "<h2>Cliquez ici pour vérifier votre compte administrateur</h2></a></div>";
    }
}
