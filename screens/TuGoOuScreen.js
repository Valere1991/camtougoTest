...
import {StyleSheet, Text,Pressable, View, TextInput, Button, ScrollView, Alert, SafeAreaView, Modal, TouchableOpacity, Platform, FlatList,Image, Avatar} from 'react-native';
...
const TuGoOuScreen = ({navigation}) => {
  useEffect(() => {
    ...
    const changeTravelCamtougo = (val) => {
        setdata({
            ...data,
            travelCamtougo: val,
            travelPicker: true,
  ...
  const [date, setDate] = useState(new Date());
    ...
    const [showDateTime, setShowDateTime] = useState(false);
     ...
     const [showSelectedTravel, setShowSelectedTravel] = useState([0])
    ...
    const fetchTravels = () => {
       axios.get(SEARCH_VOYAGE, {params})
            .then(res => {
              const val = res.data;
        ...
        return(
            <>
                ...
                <View style={{ flexDirection: 'row', padding: 12 }}>
                  <View>
                    <Image
                      style={{width: 90, height: 90, borderRadius: 100, backgroundColor: "#fff", justifyContent: 'center'}}
                      source={{uri : item.user.photo }}
                    />
                    <Text style={{fontSize: 12, alignItems: 'center', justifyContent: 'center', paddingTop: 5}}> {item.user.username}</Text>
                    <Text style={{fontSize: 10, alignItems: 'center', justifyContent: 'center', paddingTop: 5}}><Text style={{fontWeight: 'bold'}}>Nombre de places:</Text> {item.numberOfPlace}</Text>
                  </View>
                  <View style={{ marginLeft: 10, padding: 12}}>
                    <Text style={{fontSize: 18, fontWeight: 'bold', paddingLeft: 12}}>{item.startCity} - {item.endCity}</Text>
                    <Text style={{fontSize: 12, fontWeight: 'bold', paddingLeft: 12, color: '#82B123'}}>Jour: {item.travelDate} | Heure: {item.travelTime}</Text>
                    <Text style={{fontSize: 12, paddingTop: 5}}> <Text style={{fontWeight: 'bold'}}>Prix d'une place:</Text>  {item.travelPrice} FCFA</Text>
                    <Text style={{fontSize: 12, width: '60%', paddingTop: 5}}> <Text style={{fontWeight: 'bold'}}>Baggages acceptés:</Text> {item.baggageDetails}</Text>
                    <Text style={{fontSize: 12, width: '60%', paddingTop: 5}}> <Text style={{fontWeight: 'bold'}}>Détailles sur le lieu de départ:</Text> {item.startPoint}</Text>
                    <Text style={{fontSize: 12, width: '60%', paddingTop: 5}}> <Text style={{fontWeight: 'bold'}}>Informations sur la voiture:</Text> {item.carDetails}</Text>
                  </View>
                </View>
                </TouchableOpacity>
            </>
    ...
    return (
        ...
        <Text style={styles.text_header}>{t('searchTravelTitle')}</Text>
        <ScrollView>
            ...
            >
                <SafeAreaView>
                    <View style={styles.headerModal}>
                        ...
                        <Text style={styles.headerTitle}>Sélectionnez un voyage</Text>
                        <TouchableOpacity onPress={()=> setModalVisible2(false)}>
                            <Text style={styles.headerCancel}>Annuler</Text>
                        </TouchableOpacity>
                    </View>
