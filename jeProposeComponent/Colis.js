...
import {CAMCITY, CAMWORKTIME, CAMCOLIS,} from '../service/constants';

const Colis = ({navigation}) => {
    ...
    const textInputChangeShipping = (val) => {
    ...
    const textInputChangeShippingPrice = (val) => {
    ...
    const textInputChangeStartPoint = (val) => {
        if( val.trim().length >= 1 ) {
            setdata({
                ...data,
                startPoint: val,
    ...
    const textInputChangeCarDetails = (val) => {
    ...
    const handleSubmit = () => {
        if ( data.startCity.length == 0 | data.travelDate.length ==0 | data.travelTime.length ==0 |
            data.endCity.length ==0 | data.shipping.length ==0 | data.shippingPrice.length ==0 |
            data.startPoint.length ==0 | data.carDetails.length ==0 | data.colisDetails.length ==0) {
            Alert.alert('Remplir les chant!', 'Veuillez remplir tous les champs svp.', [
                {text: 'Okay'}
            ]);
            return;
        } else if ( data.startPoint.length < 12) {
            Alert.alert('Plus de détails', 'Veuillez donner plus de détails sur le lieu exacte de départ svp.', [
                {text: 'Okay'}
            ]);
            return;
        } else if ( data.carDetails.length < 12) {
            Alert.alert('Plus de détails', 'Veuillez donner plus de détails sur le véhicule svp.', [
                {text: 'Okay'}
            ]);
            return;
        ...
        if(data.startCity.length !== 0 | data.travelDate.length !==0 | data.travelTime.length !==0 |
            ...
            data.startPoint.length !==0 | data.carDetails.length !==0 | data.colisDetails.length !==0)
            {
            AuthService.colis(data)
            .then(res => {
                setShowWarning(true);
            })
            .catch(err => {
                Alert.alert('Vérifiez vos données svp!', "Rassurez-vous d'avoir bien remplit les champs svp.", [
                {text: 'Okay'}
            ]);
            })
        }
  ...
  const [date, setDate] = useState(new Date());
    ...
    const searchFilter3 = (text3) => {
        if (text3) {
            const newData3 = masterData3.filter((item) => {
                const itemData3 = item.colisDataDetails;
            });
    ...
    const ItemView3 = ({item}) => {
        return(
            <>
                <TouchableOpacity
                    onPress={() => {
                        colisDetailsPicker(item.colisDataDetails);
                        setSelected3(item.colisDataDetails);
                        setVal3(item.colisDataDetails);
                        setModalVisible3(false);
                    }}
                    style={[styles.optionContainer, {backgroundColor: item.colisDataDetails == selected3 ? '#eee' : '#fff'},
                    ]}

                >
                <Text style={[styles.optionTxt, {fontWeight: item.colisDataDetails == selected3 ? 'bold' : 'normal'},
                    ]}>
                        {item.colisDataDetails}
                    </Text>
                    {item.colisDataDetails == selected3 && (
                        <Icon name={'check'} size={22} color={'#82B123'} />
                    )}
                </TouchableOpacity>
            </>
    ...
    return (
            ...
            <View style = {styles.centered_view}>
                <View style = {styles.warning_modal}>
                    <View style={styles.titel_confirmation}>
                        <Text>SERVICE EN LIGNE!</Text>
                    </View>
                    <View style={styles.view_text}>
                        <Text style={styles.titel_text}>
                            FELICITATION! Votre service est maintenant disponible sur la plateforme. Vous serez notifié après chaque réservation. Restez connecté afin de pouvoir confirmer les réservation dans les temps svp !
                        </Text>
                    </View>
        ...
        <Text style={styles.text_header}>JE LIVRE UN COLIS</Text>
        <ScrollView>
                ...
                <Text style={styles.text_footer}>HEURE DE DÉPART</Text>
                    <TouchableOpacity style={styles.action} onPress={()=> setModalVisible2(true)}>
                        ...
                        <Text
                            ...
                            onChangeText={(val) => travelTimePicker(val)}
                        >
                            {data.travelTime}
                        </Text>
                    </TouchableOpacity>
            ...
            <View style={styles.rowP}>
                ...
                <View style={styles.actionP}>
                    <Text style={styles.text_footer}>LIEU DE LIVRAISON</Text>
                    {/* <CheckBox
                      containerStyle={{marginLeft: 0, width: '100%'}}
                      title={'Je livre à domicile'}
                      checked={circleCheckBoxValue}
                      checkedIcon='dot-circle-o'
                      uncheckedIcon='circle-o'
                      onPress={() => setCircleCheckBoxValue(!circleCheckBoxValue)}
                    /> */}
                    <View style={styles.action1C}>
                        <TextInput
                            placeholder="Précisez un lieu de livraison svp !"
                            value={data.shipping}
                            multiline
                            autoCapitalize="none"
                            onChangeText={(val)=> textInputChangeShipping(val)}
                        />
                        <Text style={styles.actionPtext}>Ex: À l'agence de voyage</Text>
                ...
                <View style={styles.actionP}>
                    ...
                    <View style={styles.action1}>
                        <TextInput
                            ...
                            autoCapitalize="none"
                            onChangeText={(val)=> textInputChangeShippingPrice(val)}
                        />
            ...
            <View style={styles.action4}>
                <TextInput
                    ...
                    numberOfLines={4}
                    autoCapitalize="none"
                    onChangeText={(val)=> textInputChangeStartPoint(val)}
                />
            </View>
            ...
            <View style={styles.action4}>
                <TextInput
                    ...
                    numberOfLines={4}
                    autoCapitalize="none"
                    onChangeText={(val)=> textInputChangeCarDetails(val)}
                />
            </View>

            <Text style={styles.text_footer}>PRÉCISION SUR LES COLIS</Text>
            <TouchableOpacity style={styles.action} onPress={()=> setModalVisible3(true)}>
                ...
                <Text
                    style={styles.txt}
                    numberOfLines={1}
                    value={data.colisDetails}
                    onChangeText={(val) => colisDetailsPicker(val)}
                >
                    {data.colisDetails}
                </Text>
            </TouchableOpacity>
            ...
            >
                <SafeAreaView>
                    <View style={styles.headerModal}>
                        ...
                        <Text style={styles.headerTitle}>Genre de colis</Text>
                        <TouchableOpacity onPress={()=> setModalVisible3(false)}>
                            <Text style={styles.headerCancel}>Annuler</Text>
                        </TouchableOpacity>
                    </View>
             ...
             <TouchableOpacity
                    ...
                    <LinearGradient
                        ...
                        style={styles.signIn}
                    >
                        <Text style = {[styles.textSign]}>LIVRER UN COLIS</Text>
                    </LinearGradient>
                </TouchableOpacity>
