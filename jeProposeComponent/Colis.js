  ...
  const [colisDetails, setColisDetails] = useState('');
    ...
    const handleSubmit = () => {
      if ( startCity === '' | travelDate === '' | travelTime === '' |
        endCity === '' | shipping === '' | shippingPrice === '' |
        startPoint === '' | carDetails === '' || colisDetails === '' ) {
      ...
      if( startCity !== '' | travelDate !== '' | travelTime !== '' |
        ...
        startPoint !== '' | carDetails !== '' || colisDetails !== '')
      {
        AuthService.colis(data.user.id, startCity, travelDate, travelTime,
          endCity, shipping, shippingPrice, startPoint,
          carDetails, colisDetails)
          .then(res => {
            setShowWarning(true);
          })
          .catch(err => {
    ...
    const ItemView3 = ({item}) => {
        return(
            <>
                <TouchableOpacity
                    onPress={() => {
                        setColisDetails(item.colisDataDetails);
        ...
        <ScrollView>
                ...
                <View style={styles.actionP}>
                    ...
                    <View style={styles.action1C}>
                        <TextInput
                            ...
                            multiline
                            autoCapitalize="none"
                            onChangeText={(shipping)=> setShipping(shipping)}
                        />
                ...
                <View style={styles.actionP}>
                    ...
                    <View style={styles.action1}>
                        <TextInput
                            ...
                            autoCapitalize="none"
                            onChangeText={(shippingPrice)=> setShippingPrice(shippingPrice)}
                        />
            ...
            <TouchableOpacity style={styles.action} onPress={()=> setModalVisible3(true)}>
                ...
                <Text
                    ...
                    numberOfLines={1}
                    value={colisDetails}
                    onChangeText={(colisDetails) => setColisDetails(colisDetails)}
                >
                    {colisDetails}
                </Text>
            </TouchableOpacity>
