...
const Travel = ({navigation}) => {
  ...
  const [baggageDetails, setBaggageDetails] = useState('');
    ...
    const handleSubmit = () => {
         if ( startCity.length ==0 | travelDate.length ==0 | travelTime.length ==0 |
            endCity.length ==0 | numberOfPlace.length ==0 | travelPrice.length ==0 |
            startPoint.length ==0 | carDetails.length ==0 | baggageDetails.length ==0 ) {
            Alert.alert(t('fullFieldTitle'), t('fullFieldText'), [
                {text: 'Okay'}
            ]);
            return;
        } else if ( startPoint.length < 12) {
            Alert.alert(t('moreDetails'), t('PlaceMoreDetails'), [
                {text: 'Okay'}
            ]);
            return;
        } else if ( carDetails.length < 12) {
            Alert.alert(t('moreDetails'), t('carMoreDetails'), [
                {text: 'Okay'}
            ]);
            return;
        ...
        <ScrollView>
            ...
            <TouchableOpacity style={styles.action} onPress={()=> setModalVisible1(true)}>
                ...
                <Text
                    ...
                    numberOfLines={1}
                    value={endCity}
                    onChangeText={(endCity) => setEncity(endCity)}
                >
                    {endCity}
                </Text>
            </TouchableOpacity>
